...
import {
  chatIdentities, conversations, participants, messages,
  notifications, statusUpdates, calls, announcements,
  faculties, departments, staff, students, users,
  type ChatIdentity, type Conversation, type Message, type Participant,
...
export class DatabaseStorage implements IStorage {
  ...
  async getConversations(identityId: number): Promise<(Conversation & { lastMessage?: Message, unreadCount: number, participants: Participant[] })[]> {
    ...
    for (const chat of chats) {
        ...
        const parts = await this.getParticipants(chat.id);
  ...
  // --- Messages ---
  async getMessages(conversationId: number, limit: number = 50, cursor?: number): Promise<Message[]> {
    const conditions = [eq(messages.conversationId, conversationId)];
    // Cursor logic implementation omitted for brevity, focusing on core
